...
from .models import UserProfile, Vehicle, Review, Favorite, Payment, Booking
...
class VehicaleSerializer(serializers.ModelSerializer):
    ...
    def get_is_booked(self, obj):
        ...
        if not start_at or not end_at:
            return False
        parse_start_at = parse_datetime(start_at)
        parse_start_at=parse_datetime(end_at)
        if not parse_start_at or not parse_start_at:
            return False
        return obj.bookings.filter(
            start_at__lt=parse_start_at,
            end_at__gt=parse_start_at,
            status__in=[Booking.PENDING, Booking.STATUS_CONFIRMED]
        ).exists()
    
    def get_images(self, obj):
        return [f"http://localhost:8000{image.image.url}" for image in obj.images.all()]

...
class Booking(models.Model):
    ...
    booking_id=models.CharField(max_length=11,unique=True,editable=False)
    ...
    delivery_fee=models.DecimalField(max_digits=10, decimal_places=2, default=Decimal("0.00"))
    pricing_unit=models.CharField(max_length=20, choices=UNIT_CHOICES)
    ...
    #     return delta.total_seconds() / Decimal("3600")
...
def calculate_total(self) -> Decimal:
    ...
    if self.pricing_unit == self.UNIT_HOURLY:
        base = hours * self.vehicle.hourly_rate

    elif self.pricing_unit == self.UNIT_DAILY:
        days = Decimal(math.ceil(hours / Decimal(24)))
        base = days * self.vehicle.daily_rate

    else:
        weeks = Decimal(math.ceil(hours / Decimal(24 * 7)))
        base = weeks * self.vehicle.weekly_rate

    return (base + delivery_fee).quantize(Decimal("0.01"))

def save(self, *args, **kwargs):
    if timezone.is_naive(self.start_at):
        self.start_at = timezone.make_aware(self.start_at)
    if timezone.is_naive(self.end_at):
